...
_request_element_list = ["Workload", "Auth_login", "VerifyCode_verifyCode", "Admin_getAllUser", "User_getAllUser",
                         "Admin_updateAllUser", "User_updateAllUser", "VerifyCode_generateCode", "Order_findAllOrder",
                         "Rebook_rebook", "Order_getOrder"]

_simplize_request_element_list = ["Workload", "s5/url_1", "s6/url_1", "s7/url_2", "s8/url_2", "s7/url_3", "s8/url_3",
                                  "s6/url_4", "s9/url_5", "s10/url_6", "s9/url_6"]
...
def get_box_values(model_file_address="./ori/final_model.pkl", subject="ffmpeg"):
    ...
    # tc.get_config_space()
    tc.get_config_space("s9", f"{subject}_test_only")
...
def generate_and_update(n_cases=20, children_limit=20):
    ...
    for i in range(n_cases):
        ...
        _rank_list, _mean_st_list = qpme_running(change_list, _qpme_file_location="./qpme/trainticket_model.qpe")
        data_package = {
            "case_id": i,
            "seed_id": 0,
...
def main():
    generate_and_update(n_cases=len(frequency_list), children_limit=20)
