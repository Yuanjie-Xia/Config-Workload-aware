...
def active_learning_model():
    for url in _url_type:
        work_model_address = "./muBench_changed_files/trainticket/WorkModel.json"
        ...
        _final_model = active_learning_committee_dp(tc, deployer, url, fine_tune_every=1)
...
def active_learning_w_qpme():
    ...
    for url in _url_type:
        ...
        for index in range(0, 18):
            real_index = k * 18 + index
            print("------------------------------")
            _location_dict = {
                "ffmpeg": "./ori/final_model_18.pkl"
            }
            change_elements, ranked_list, mean_st_ranked_list = qpme_process(
                _location_dict, tc.config_space, real_index, "train", mix_status=False)
            # change_elements, ranked_list, mean_st_ranked_list = qpme_process("./ori/final_model.pkl", tc.config_space, index, "train")
