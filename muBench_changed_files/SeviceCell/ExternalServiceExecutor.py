...
def external_service(group,id,work_model,trace,query_string, app, trace_context):
    ...
    for service in selected_services:
        ...
        try:
            ...
            if random.random() < p :
                ...
                response_bodies[service] = r.text
    ...
    return service_error_flag, service_error_dict, response_bodies
...
def run_external_service(services_group, work_model, query_string, trace, app, trace_context=None):
    ...
    futures = list()
    id = 0
    request_type = trace_context.get('X-Requesttype')

    if request_type not in services_group:
        app.logger.error(f"Ended since X-Requesttype not exist: {request_type}")
        return {}, {}  # or appropriate error handling
    selected_groups = services_group[request_type]
    for group in selected_groups:
        futures.append(pool.submit(external_service, group, id, work_model, trace, query_string, app, trace_context))
        id = id + 1
    wait(futures)
    for x in as_completed(futures):
        service_output.update(x.result()[2])
        if x.result()[0]:
            service_error_dict.update(x.result()[1])
    app.logger.info("--------> Threads Done!")
    return service_error_dict, service_output