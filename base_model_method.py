...
def active_learning_model():
    for url in _url_type:
        ...
        _final_model = active_learning_committee(tc, deployer, url)
...
def active_learning_w_qpme():
    ...
    for url in _url_type:
        ...
        for index in range(0, 110):
            real_index = k * 110 + index
            print("------------------------------")
            change_elements, ranked_list, mean_st_ranked_list = qpme_process("./ori/final_model.pkl", tc.config_space, real_index)
