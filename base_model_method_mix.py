...
def active_learning_committee(test_case, deployer, url, rounds=100, n_committee=5):
    ...
    for i in range(len(init_configs), rounds):
        ...
        if i > 10:
            ...
            with open(f"modAL_all_{url}_teastore.pkl", "wb") as f:
...
def active_learning_committee_dp(test_case, deployer, url, rounds=100, n_committee=5, fine_tune_every=3, verbose_tune=1):
    ...
    for i in range(len(init_configs), rounds):
        ...
        if i > 10:
            ...
            model_path = f"deepperf_cv_{url}_teastore.pt"
...
def active_learning_model():
    for url in _url_type:
        ...
        random.seed(42)
        deployer = Deployer(['s0', 's1', 's2', 's3', 's4'], tc.workload_json)
        ...
        _final_model = active_learning_committee_dp(tc, deployer, url, rounds=200)
        with open(f"final_model_{url}_teastore.pkl", "wb") as f:
...
def active_learning_w_qpme():
    ...
    for url in _url_type:
        ...
        tc_1.get_config_space("s4", "test_only")
        random.seed(42)
        deployer = Deployer(['s0', 's1', 's2', 's3', 's4'], tc.workload_json)
        # deployer.update_dns_setting()
        time.sleep(3)
        ...
        for index in range(0, 38):
            real_index = k * 38 + index
            print("------------------------------")
            _location_dict = {
                "ffmpeg": "./ori/final_model_38.pkl",
                "lrzip": "./ori/lrzip_final_model_38.pkl"
            }
            change_elements, ranked_list, mean_st_ranked_list = qpme_process(
                _location_dict, tc.config_space, real_index, "teastore", mix_status=True)
        ...
        with open(f"method_results_{url}_teastore.pkl", "wb") as f:
            pickle.dump(save_data, f)
        k = k + 1
